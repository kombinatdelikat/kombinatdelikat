...
class ContentAccountingSubtotal extends ContentAccounting
{
	...
	protected function compile()
	{
		...
		$objElements = \Contao\ContentModel::findBy(
			array('id!=? AND pid=? AND ptable=? AND type=? AND sorting<=?'),
			array($this->id, $this->pid, $this->ptable, 'accounting_item', $this->sorting)
		);
